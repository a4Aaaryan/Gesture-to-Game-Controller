...
class ArmsState:
    ...
    def update(
        ...
        if mode == "Driving":
            if is_landmarks_closed(
                ...
                cv2.circle(
                    image,
                    (
                        int((left_wrist[0] + right_wrist[0]) / 2 * IMAGE_WIDTH),
                        int((left_wrist[1] + right_wrist[1]) / 2 * IMAGE_HEIGHT),
                    ),
                    50,
                    (255, 0, 0),
                    5,
                ...
                if p%10 == 0:
                    if is_landmarks_in_rectangle(
                        ...
                        **DRIVING_UP_AREA,
                        ...
                        if right_thumb_angle < 10 and right_thumb_angle is not None:
                            events.add("nitro")
                        if left_thumb_angle < 10 and left_thumb_angle is not None:
                            events.add("brakes")
                        else:
        ...
        if (
            compare_nums(left_wrist[0], right_wrist[0], "gt")
        ...
        if self.left.raised and self.right.raised:
            thread = threading.Thread(target=press_up_arrow)
            thread.start() 
            events.add("squat")

        if right_shoulder_angle > 40:
            self.stage = "up"
        if right_shoulder_angle < 25 and self.stage == "up":
            self.stage = "down"
            events.add("flap")
