    ...
    def calculate(self, image, results):
        ...
        try:
            ...
            if results.left_hand_landmarks is not None and results.right_hand_landmarks is not None:
                ...
                left_thumb_tip =get_landmark_coordinates(
                    left_hand_landmarks, mp_hands.HandLandmark.THUMB_TIP
                )
                right_thumb_tip =get_landmark_coordinates(
                    right_hand_landmarks, mp_hands.HandLandmark.THUMB_TIP
                )

                left_thumb_cmc =get_landmark_coordinates(
                    left_hand_landmarks, mp_hands.HandLandmark.THUMB_CMC
                )
                right_thumb_cmc =get_landmark_coordinates(
                    right_hand_landmarks, mp_hands.HandLandmark.THUMB_CMC
                )

                left_index_mcp =get_landmark_coordinates(
                    left_hand_landmarks, mp_hands.HandLandmark.INDEX_FINGER_MCP
                )
                right_index_mcp =get_landmark_coordinates(
                    right_hand_landmarks, mp_hands.HandLandmark.INDEX_FINGER_MCP
                )

                left_thumb_angle = calculate_angle(left_thumb_tip, left_thumb_cmc, left_index_mcp)
                right_thumb_angle = calculate_angle(right_thumb_tip, right_thumb_cmc, right_index_mcp)
            ##################
